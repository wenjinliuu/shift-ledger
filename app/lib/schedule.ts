...
export type WorkSystem = "standard" | "comprehensive" | "irregular" | "custom" | "manual";
export type StatisticsPeriod = "week" | "month" | "quarter" | "halfYear" | "year" | "custom";
export type CompensationMode = "hours" | "salary" | "timeOff" | "fixed" | "custom" | "none";
...
function shiftCatalog(hours: Partial<Record<string, number>> = {}): Shift[] {
  return [
    ...
    { id: SHIFT_IDS.morning, name: "早班", shortName: "早", color: "#e89135", startTime: "08:00", endTime: "16:00", crossesMidnight: false, isRest: false, defaultHours: hours.morning ?? 8, countsAsWork: true, legacyType: "morning" },
    { id: SHIFT_IDS.middle, name: "中班", shortName: "中", color: "#08a2b8", startTime: "16:00", endTime: "00:00", crossesMidnight: false, isRest: false, defaultHours: hours.middle ?? 8, countsAsWork: true, legacyType: "middle" },
    { id: SHIFT_IDS.late, name: "晚班", shortName: "晚", color: "#7459d9", startTime: "00:00", endTime: "08:00", crossesMidnight: false, isRest: false, defaultHours: hours.late ?? 8, countsAsWork: true, legacyType: "late" },
    { id: SHIFT_IDS.rest, name: "休息", shortName: "休", color: "#8793a5", startTime: "", endTime: "", crossesMidnight: false, isRest: true, defaultHours: 0, countsAsWork: false, legacyType: "rest" },
...
export function builtInTemplates(): CycleTemplate[] {
  ...
  return [
    { id: "tpl-four-two", name: "4白2休 · 4夜2休", caption: "白白白白休休 · 夜夜夜夜休休", shiftIds: [day, day, day, day, rest, rest, night, night, night, night, rest, rest], category: "manufacturing", builtIn: true },
    { id: "tpl-two-rest-two", name: "2白2休 · 2夜2休", caption: "白白休休 · 夜夜休休", shiftIds: [day, day, rest, rest, night, night, rest, rest], category: "manufacturing", builtIn: true },
    { id: "tpl-one-one-two", name: "1白1夜 · 休2天", caption: "白夜休休", shiftIds: [day, night, rest, rest], category: "manufacturing", builtIn: true },
    { id: "tpl-two-two-two", name: "2白2夜 · 休2天", caption: "白白夜夜休休", shiftIds: [day, day, night, night, rest, rest], category: "manufacturing", builtIn: true },
    { id: "tpl-work-two-rest-two", name: "做二休二", caption: "白白休休", shiftIds: [day, day, rest, rest], category: "manufacturing", builtIn: true },
    { id: "tpl-three-four", name: "3上4休 / 4上3休", caption: "白白白休休休休 · 白白白白休休休", shiftIds: [day, day, day, rest, rest, rest, rest, day, day, day, day, rest, rest, rest], category: "manufacturing", builtIn: true },
    { id: "tpl-three-shift", name: "早 → 中 → 夜 → 休", caption: "早中晚休", shiftIds: [morning, middle, late, rest], category: "threeShift", builtIn: true },
    { id: "tpl-double-three", name: "夜夜 → 中中 → 早早 → 休休", caption: "晚晚中中早早休休", shiftIds: [late, late, middle, middle, morning, morning, rest, rest], category: "threeShift", builtIn: true },
    { id: "tpl-four-team-three-shift", name: "四班三倒 · 8小时", caption: "早早中中晚晚休休", shiftIds: [morning, morning, middle, middle, late, late, rest, rest], category: "threeShift", builtIn: true },
  ];
}
...
export function calculateShiftDuration(startTime: string, endTime: string, crossesMidnight: boolean) {
...
function normalizeShift(raw: unknown): Shift | null {
  ...
  return {
    ...
    shortName: typeof item.shortName === "string" && item.shortName.trim() ? item.shortName.trim().slice(0, 4) : item.name.slice(0, 2),
    ...
    legacyType: typeof item.legacyType === "string" ? item.legacyType : undefined,
  };
}

function normalizeRecord(raw: unknown, validShiftIds: Set<string>, validTagIds: Set<string>): DayRecord | null {
  if (!raw || typeof raw !== "object") return null;
  const item = raw as Record<string, unknown>;
  const legacyShift = typeof item.shift === "string" ? LEGACY_SHIFT_MAP[item.shift] : "";
  ...
  return {
    ...
    tagIds: Array.isArray(item.tagIds) ? item.tagIds.filter((id): id is string => typeof id === "string" && validTagIds.has(id)) : [],
    completed: Boolean(item.completed),
    planned: item.planned !== false,
    note: typeof item.note === "string" ? item.note : undefined,
    manualOvertime: numberValue(item.manualOvertime, 0),
    source: item.source === "cycle" || item.source === "manual" || item.source === "legacy" ? item.source : "legacy",
...
export function normalizeAppData(raw: unknown): AppData {
  ...
  const item = raw as Record<string, unknown>;
  if (item.dataVersion !== DATA_VERSION || !Array.isArray(item.shifts)) return migrateLegacyData(item.settings, item.records);
  const fallback = createDefaultData();
  const shifts = uniqueById(item.shifts.map(normalizeShift).filter((shift): shift is Shift => Boolean(shift)));
  const safeShifts = shifts.length ? shifts : fallback.shifts;
  const tags = uniqueById((Array.isArray(item.tags) ? item.tags : []).flatMap((rawTag): DutyTag[] => {
    ...
    return [{ id: tag.id, name: tag.name.trim() || "未命名标签", shortName: typeof tag.shortName === "string" ? tag.shortName.trim().slice(0, 6) : tag.name.slice(0, 3), color: typeof tag.color === "string" ? tag.color : TAG_COLORS[0] }];
  }));
  ...
  const templates = uniqueById((Array.isArray(item.cycleTemplates) ? item.cycleTemplates : builtInTemplates()).flatMap((rawTemplate): CycleTemplate[] => {
    ...
    const ids = Array.isArray(template.shiftIds) ? template.shiftIds.filter((id): id is string => typeof id === "string" && shiftIds.has(id)) : [];
    if (typeof template.id !== "string" || typeof template.name !== "string" || !ids.length) return [];
    const category = template.category === "threeShift" || template.category === "medical" || template.category === "custom" ? template.category : "manufacturing";
    return [{ id: template.id, name: template.name, caption: typeof template.caption === "string" ? template.caption : "", shiftIds: ids, category, builtIn: Boolean(template.builtIn) }];
  }));
  const activeRaw = item.activeCycle && typeof item.activeCycle === "object" ? item.activeCycle as Record<string, unknown> : null;
  const activeIds = activeRaw && Array.isArray(activeRaw.shiftIds) ? activeRaw.shiftIds.filter((id): id is string => typeof id === "string" && shiftIds.has(id)) : [];
  const activeCycle = activeRaw && activeIds.length && typeof activeRaw.startDate === "string" ? { id: typeof activeRaw.id === "string" ? activeRaw.id : makeId("cycle"), name: typeof activeRaw.name === "string" ? activeRaw.name : "我的循环", startDate: activeRaw.startDate, shiftIds: activeIds } : null;
  const rawTargets = item.targets && typeof item.targets === "object" ? item.targets as Record<string, unknown> : {};
  const targets = Object.fromEntries(Object.entries(rawTargets).flatMap(([key, value]) => Number.isFinite(Number(value)) && Number(value) >= 0 ? [[key, Number(value)]] : []));
  return {
    dataVersion: DATA_VERSION,
    careerPreset: item.careerPreset === "medical" || item.careerPreset === "transport" || item.careerPreset === "safety" || item.careerPreset === "service" || item.careerPreset === "custom" ? item.careerPreset : "manufacturing",
    shifts: safeShifts,
    tags,
    ...
    work: {
      ...
      system: rawWork.system === "standard" || rawWork.system === "irregular" || rawWork.system === "custom" || rawWork.system === "manual" ? rawWork.system : "comprehensive",
      period: rawWork.period === "week" || rawWork.period === "month" || rawWork.period === "quarter" || rawWork.period === "halfYear" || rawWork.period === "custom" ? rawWork.period : "year",
      ...
      customRule: rawWork.customRule === "daily" || rawWork.customRule === "weekly" || rawWork.customRule === "monthly" || rawWork.customRule === "period" ? rawWork.customRule : "manual",
      customThreshold: numberValue(rawWork.customThreshold, 0),
      compensation: rawWork.compensation === "salary" || rawWork.compensation === "timeOff" || rawWork.compensation === "fixed" || rawWork.compensation === "custom" || rawWork.compensation === "none" ? rawWork.compensation : "hours",
    },
    targets,
    records: (Array.isArray(item.records) ? item.records : []).map((record) => normalizeRecord(record, shiftIds, tagIds)).filter((record): record is DayRecord => Boolean(record)).sort((a, b) => a.date.localeCompare(b.date)),
  };
}
...
export function migrateLegacyData(settingsRaw: unknown, recordsRaw: unknown): AppData {
  ...
  const rawTargets = settings.targets && typeof settings.targets === "object" ? settings.targets as Record<string, unknown> : {};
  const data = createDefaultData();
  data.shifts = shifts;
  data.work.dailyStandard = numberValue(settings.dailyStandard, 8);
  data.targets = Object.fromEntries(Object.entries(rawTargets).flatMap(([key, value]) => Number.isFinite(Number(value)) && Number(value) >= 0 ? [[key, Number(value)]] : []));
  data.activeCycle = rawCycle.length && typeof settings.cycleStart === "string" && settings.cycleStart ? { id: "cycle-migrated", name: "原有循环", startDate: settings.cycleStart, shiftIds: rawCycle.map((shift) => LEGACY_SHIFT_MAP[shift]) } : null;
  data.records = (Array.isArray(recordsRaw) ? recordsRaw : []).map((record) => normalizeRecord(record, validShiftIds, new Set())).filter((record): record is DayRecord => Boolean(record));
  return data;
}
...
export function applyCareerPreset(data: AppData, preset: CareerPreset): AppData {
  ...
  if (preset === "medical") {
    additions.push(
      { id: SHIFT_IDS.smallNight, name: "小夜", shortName: "小夜", color: "#665ce8", startTime: "16:00", endTime: "00:00", crossesMidnight: false, isRest: false, defaultHours: 8, countsAsWork: true },
  ...
  return { ...data, careerPreset: preset, shifts, tags: nextTags, cycleTemplates: templates };
}
...
export function dayDifference(a: string, b: string) {
  const aa = parseDateKey(a); const bb = parseDateKey(b);
  return Math.round((new Date(aa.getFullYear(), aa.getMonth(), aa.getDate()).getTime() - new Date(bb.getFullYear(), bb.getMonth(), bb.getDate()).getTime()) / 86400000);
}

export function generateCycleRecords(activeCycle: ActiveCycle, shifts: Shift[], startDate: string, endDate: string): DayRecord[] {
  const shiftMap = new Map(shifts.map((shift) => [shift.id, shift]));
  const start = parseDateKey(startDate < activeCycle.startDate ? activeCycle.startDate : startDate);
  const end = parseDateKey(endDate);
  const generated: DayRecord[] = [];
  for (const cursor = new Date(start); cursor <= end; cursor.setDate(cursor.getDate() + 1)) {
    const date = formatDateKey(cursor);
    const index = ((dayDifference(date, activeCycle.startDate) % activeCycle.shiftIds.length) + activeCycle.shiftIds.length) % activeCycle.shiftIds.length;
    const shiftId = activeCycle.shiftIds[index];
    const shift = shiftMap.get(shiftId);
    if (!shift) continue;
    generated.push({ date, shiftId, hours: shift.defaultHours, tagIds: [], completed: false, planned: true, source: "cycle", cycleId: activeCycle.id });
  }
  return generated;
}

export function materializeCycleYear(data: AppData, year: number): AppData {
  if (!data.activeCycle || Number(data.activeCycle.startDate.slice(0, 4)) > year) return data;
  const start = `${year}-01-01` < data.activeCycle.startDate ? data.activeCycle.startDate : `${year}-01-01`;
  const generated = generateCycleRecords(data.activeCycle, data.shifts, start, `${year}-12-31`);
  ...
  generated.forEach((record) => {
    ...
    if (!current || current.source === "cycle") existing.set(record.date, current?.source === "cycle" && current.cycleId === data.activeCycle?.id ? current : record);
  });
  return { ...data, records: [...existing.values()].sort((a, b) => a.date.localeCompare(b.date)) };
}

export function replaceCycleFromDate(data: AppData, activeCycle: ActiveCycle, throughYear: number): AppData {
  const kept = data.records.filter((record) => record.date < activeCycle.startDate);
  const generated = generateCycleRecords(activeCycle, data.shifts, activeCycle.startDate, `${throughYear}-12-31`);
  return { ...data, activeCycle, records: [...kept, ...generated].sort((a, b) => a.date.localeCompare(b.date)) };
}

export function recordsInRange(records: DayRecord[], startDate: string, endDate: string) {
...
export function calculateOvertime(records: DayRecord[], settings: WorkSettings, standardTarget: number) {
  if (!settings.trackHours || !settings.trackOvertime) return 0;
  const total = records.reduce((sum, record) => sum + Math.max(0, record.hours), 0);
  if (settings.system === "comprehensive") return Math.max(0, total - standardTarget);
  if (settings.system === "manual" || settings.system === "irregular" || (settings.system === "custom" && settings.customRule === "manual")) return records.reduce((sum, record) => sum + Math.max(0, record.manualOvertime ?? 0), 0);
  if (settings.system === "custom") {
    if (settings.customRule === "daily") return records.reduce((sum, record) => sum + Math.max(0, record.hours - settings.customThreshold), 0);
    if (settings.customRule === "weekly") {
      const weeks = new Map<string, number>();
      records.forEach((record) => weeks.set(startOfWeek(record.date), (weeks.get(startOfWeek(record.date)) ?? 0) + record.hours));
      return [...weeks.values()].reduce((sum, hours) => sum + Math.max(0, hours - settings.customThreshold), 0);
    }
    return Math.max(0, total - settings.customThreshold);
  }
  const daily = settings.standardDailyEnabled ? records.reduce((sum, record) => sum + Math.max(0, record.hours - settings.dailyStandard), 0) : 0;
  let weekly = 0;
  if (settings.standardWeeklyEnabled) {
    const weeks = new Map<string, number>();
    records.forEach((record) => weeks.set(startOfWeek(record.date), (weeks.get(startOfWeek(record.date)) ?? 0) + record.hours));
    weekly = [...weeks.values()].reduce((sum, hours) => sum + Math.max(0, hours - settings.weeklyStandard), 0);
  }
